...
2. Ejecuta `renderer/render_zpl_local.mjs` con un runtime de Node embebido en el bundle de macOS cuando existe; si no, usa `node` desde PATH.
...
Ese script ahora tambien intenta generar un `.dmg` instalable (`dist/macos-intel/ZPLConverter-x86_64.dmg`) si detecta `hdiutil`.
