...
La app intenta usar primero un runtime de Node.js embebido dentro de `renderer/bin/node` (incluido en el bundle de PyInstaller durante el build de macOS). Si no existe, usa `node` desde PATH.
...
El script `scripts/build_macos.sh` copia el binario real de `node` detectado en PATH hacia `renderer/bin/node` antes de ejecutar PyInstaller. Como `pyinstaller/main.spec` empaqueta toda la carpeta `renderer`, el bundle final incluye ese runtime junto al renderer local.

Esto permite que la aplicacion distribuida en `.app` o `.dmg` funcione en Macs Intel sin requerir que el usuario instale Node manualmente, siempre que el build se haya generado en una maquina Intel compatible.

Si necesitas omitir el DMG durante pruebas locales, ejecuta:
